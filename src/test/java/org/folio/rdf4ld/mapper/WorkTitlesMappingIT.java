...
class WorkTitlesMappingIT {
  ...
  void mapToBibframeRdf_shouldReturnMappedRdfInstanceWithTitles() throws IOException {
    ...
    var expected = new String(this.getClass().getResourceAsStream("/rdf/work_titles.json").readAllBytes())
      ...
      .replaceAll("WORK_PARALLEL_TITLE_NOTE_ID_1", parallelWorkTitle.getId().toString() + "_NOTE_1")
      .replaceAll("WORK_PARALLEL_TITLE_NOTE_ID_2", parallelWorkTitle.getId().toString() + "_NOTE_2")
      .replaceAll("WORK_VARIANT_TITLE_NOTE_ID_1", variantWorkTitle.getId().toString() + "_NOTE_1")
      .replaceAll("WORK_VARIANT_TITLE_NOTE_ID_2", variantWorkTitle.getId().toString() + "_NOTE_2")
